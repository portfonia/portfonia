...
- [§3 proportionality check: log-only length-vs-(weight, evidence)](docs/mechanisms/report-content-and-email.md) — issue #173: `section3_proportionality.py`, keyword-scored evidence strength, paragraph+identifier §3 segmentation (no per-holding heading exists), weight as an explicit parameter (issue #421 dependency), wired into `_render_full_md`.
