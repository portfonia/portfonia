...
- [Idle-timeout server enforcement](docs/mechanisms/identity-and-auth.md) — issue #235: the 15-min auto-logout (issue #207) was client-only and silently defeated by closing the browser; `current_principal` now checks a Redis-stored `(timestamp, session_id)` record (`app/core/idle_activity.py`) keyed on the JWT's `session_id` claim (not `iat` — a round-1 fix that broke on Supabase's silent background token refresh, caught in review round 2), fail-open on Redis outage (deliberate departure from rate_limit's fail-closed convention — blast radius, see mechanism doc); absolute session lifetime (Supabase refresh-token expiry) and per-user configurable length are explicitly out of scope here. PR #240 review: round 1 found 2 ship-blockers (stale idle lock survived re-login; 401 never signed the browser out — fixed by routing `lib/api.ts`/`lib/server-api.ts` 401s through the same `logout()` Server Action the client idle timer uses, across 8 call sites); round 2 found round 1's re-login fix itself unsound (`iat` changes on every silent refresh, not just a login) and required keying on `session_id` instead — see mechanism doc for the full history.
