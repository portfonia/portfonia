...
- [Shared instrument symbol normalization — instrument_symbols](docs/mechanisms/instrument-symbols.md) — issue #57 (PRs #359/#361/57-3): consolidates ticker/market normalization into one module, removes the `_yfinance` forwarding shim.
- [Portfolio Performance — Phase 1 + tracking-start + holiday history + param-limit fixes](docs/mechanisms/portfolio-performance.md) — issue #360/#382/#383/#366/#377/#398/#402/#403: snapshot tables, approximate EOD TWR, `tracking_start` correction (#366), benchmark/FX multi-year seed scripts, both scripts' 65535-param chunking fix.
...
- [Fund NAV staleness observability](docs/mechanisms/capture-and-reporting.md) — issue #298/PR #303: per-fund stale/missing NAV WARNING + ops alert, durable Redis dedup; observability only, #135 root cause still open.
