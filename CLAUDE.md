...
- [Capture-health human-readable alert + FX catch-up (retry + Twelve Data fallback)](docs/mechanisms/portfolio-performance.md) — issue #426: `CaptureHealthReport` carries each pipeline's last-success date, alert body rewritten to plain-English blocks; `capture_fx_catchup_task` (00:05 ET `tue-sat`) recovers a prior-day FX gap via one retry then Twelve Data, widening `TWELVEDATA_API_KEY`'s scope beyond the #406 USDCNH gap-fill.
