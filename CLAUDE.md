...
- [Portfolio overview dashboard — C2](docs/mechanisms/holdings-pipeline.md) — issue #320 (#130 C2), PR #322: `/portfolio` reads `GET /portfolio/summary`; `by_group` aggregates `Holding.portfolio`, `by_broker` (renamed from `by_account` — see the issue #330 entry below) aggregates `Holding.broker` **not** the normalized `accounts` table (deliberate — `accounts`' dedup key is the `(broker, account, portfolio)` triple, so reading `account_id` would fold the group dimension back into account); P&L (`cost_basis_base`/`unrealized_pnl_base`/`unrealized_pnl_pct`) computed only for `pricing_mode=="auto"` with both `shares` and `avg_cost` and a valuation, `None` (not zero) otherwise — cash/wmf and `capture_supported=False` holdings never get one; frontend partitions the "no market quote" section on `pricing_mode=="auto" and not capture_supported`, never on the broader `market_value_base is None`; `base_currency` widened to all 15 `VALID_CURRENCIES`; `price_as_of_date` is the max captured-close trade date actually used (`None` on a cash-only book — the as-of banner has separate copy for that case, not "no priced holdings").
- [Portfolio dashboard: by-account breakdown, currency display modes, sector removal](docs/mechanisms/holdings-pipeline.md) — issue #330, PR #332: renames the C2-era `by_account` (keyed on `Holding.broker`) to `by_broker`, adds a genuine `by_account` keyed on the free-text `Holding.account`; currency card gains a frontend-only 本币/归一/比例 toggle (no backend change — `HoldingValueOut` already carries native currency/value; **this toggle was removed and replaced by issue #350 item 2's unified single-list row format** — see that entry below); by-sector (GICS) card removed from the UI only, `by_sector` untouched in the backend. Two round-1 review fixes: the native-mode pie no longer sizes arcs from mixed-currency raw numbers (list-only there via a new `showPie` prop), and native-mode membership now matches normalized mode's `market_value_base is not None` gate (a stale-FX holding no longer appears in one mode and vanishes in another).
- [Currency & language cleanup batch — per-user currency, by-currency card, cash P&L, as-of banner](docs/mechanisms/holdings-pipeline.md) — issue #350 items 1/2/5/6: `users.base_currency` gets a CHECK constraint + `report_currency_for()`, `PATCH /me/report-currency` + ops sibling, threaded through self-service generate/regenerate/admin-rerun/scheduled fan-out, and `GET /portfolio/summary` defaults an omitted `base_currency` to it (seeds the frontend switcher's initial value); the by-currency card's native/normalized/percentage mode dropdown (issue #330) is replaced by one unified row per currency (`{native} / {base} ({share}%)`, `formatCurrencyBreakdownRow()`); portfolio-level `total_cost_basis_base` now includes cash/wmf's `market_value_base` (never `total_unrealized_pnl_base`'s numerator) so cash dilutes but doesn't move total P&L%; the price-as-of banner's copy now frames the date as each market's own latest close, not one global timestamp. Item 3 (report footer disclaimer) is in `report-content-and-email.md`; item 4 (header locale switcher) has no dedicated mechanism doc, see the Frontend chrome entry above.
- [Portfolio snapshot export: xlsx and md](docs/mechanisms/holdings-pipeline.md) — issue #331, PR #335: `GET /portfolio/export?format=xlsx|md`, a read-only sibling of `/holdings/export` (declared/unpriced fields for re-import) that instead serializes `compute_portfolio()`'s computed, priced per-holding rows — no `by_*` aggregates, `as_of`/`base_currency` as header metadata only. New `portfolio_export.py` module (not shared with `holdings_export.py`'s `#####` re-import dialect); `openpyxl` was already an unused pin in `requirements.txt`. Review fix (5103601953): `unrealized_pnl_pct` is a 0..1 ratio in `compute_portfolio()`, not a percent — scaled by 100 at render time to match every other consumer (`formatPercent`, the overview email's `:.1%`).
...
- [FX currency coverage + ticker-normalization consistency](docs/mechanisms/capture-and-reporting.md) — issue #204/PR #253: `_CURRENCY_TO_FX_PAIR`/`fx_fetcher._PAIRS` widened from 3 to all 14 `VALID_CURRENCIES`; a ticker-collision override table (`PSH -> PSH.L`, historically `_yfinance._TICKER_SYMBOL_OVERRIDE`, now owned by `instrument_symbols` — see the next entry) composes with the existing HK normalizer; every consumer that derives a lookup/join key from a raw `Holding.ticker` must call it — full list of call sites and the 3 rounds of review findings that surfaced them in the mechanism doc. The GBX subunit correction that originally lived here as a per-ticker `_TICKER_PRICE_SCALE` table was superseded by #311/PR #312's generic `currency == "GBp"` detection (see the next-but-one entry) — the ticker-collision override is a separate, still-current mechanism. #252 (no scheduled capture for `market="Other"`) closed via #311/PR #312. Issue #351/PR #352 found one more missed consumer of the normalization invariant (`price_capture.capture_prices`'s own `missing`-list check, both as a lookup key and as the fallback call's input) and, separately, that PR #310's currency-based suffix-forcing never retroactively fixes a holding written before it existed — the actual production PSH row was corrected directly rather than via a new general backfill script (product owner call: one known-bad row, disproportionate blast radius for a general sweep).
- [Shared instrument symbol normalization — instrument_symbols](docs/mechanisms/instrument-symbols.md) — issue #57, three sequential PRs (57-1 #359, 57-2 #361, 57-3): consolidates ticker/fund-code normalization, market/suffix classification, and the intelligence-cache identifier convention (`intelligence_identifier`) into one module, replacing the formerly duplicated `_yfinance._normalize_ticker`/`_TICKER_SYMBOL_OVERRIDE` and per-consumer reimplementations in `window_data`/`ticker_intel`/`report_assembly`/`user_scope`/`ticker_leverage`. 57-3 removed the `_yfinance` forwarding shim entirely (zero business imports remain — see the mechanism doc's zero-dependency check) and widened `frontend/src/lib/api.ts`'s `Market` type to the full 8-value taxonomy. A known pre-#57 baseline discrepancy (fund_code-before-ticker precedence in `_confirmed_market`) is preserved as-is, not repaired, per the frozen design.
- [Portfolio Performance — Phase 1 + #366 tracking-start + #377 holiday history](docs/mechanisms/portfolio-performance.md) — issue #360 / #382 / #383: `portfolio_value_snapshots`/`portfolio_snapshot_batches`/`benchmark_prices` (all new, no change to `holdings`/`/portfolio/summary`), two daily Beat tasks (20:30 ET, after every market close + the FX fetch), approximate EOD TWR (`r_t` marks yesterday's filtered holdings at today's own stored per-holding row — a full exit or a market/broker filter drop reads as cash flow, never a return; group/account regroup follows current attribution), `GET /portfolio/performance` (filters AND'd; D8: group/account = current live labels via soft `holding_id`, market/broker = snapshot-day denorm; issue #371). Phase 2 (frontend/chart) is a separate follow-up PR against this frozen response contract. **Issue #382**: first visit is range `1M` + benchmarks `[sp500]`; empty chips stay portfolio-only (no silent expand to the full catalog). **Issue #383** (closed): catalog adds `csi300` (CSI 300 / yfinance `000300.SS` / CNY, price index, PR #385); China A50 is shelved — not on the near-term agenda. **Issue #366 (2026-09-07 design amendment, supersedes D2)**: production verification found Phase 1's one-off portfolio backfill derived a position's chart start from ticker price availability, not from when the user was actually tracked (~35,605 fictional rows, starts in 2024-11/12 for a mid-2026 product) — `backfill_portfolio_value_history.py` is **deleted outright** (never a product path again), `portfolio.tracking_start` = first real (`is_backfilled=False`) complete-batch day, `is_backfilled` rows are excluded from every aggregate/TWR link/empty-check, and benchmark cumulative % is co-anchored to the portfolio start when that day can be valued (issue #377 keeps selected-range index history and splits `displayable` from `comparable`; non-comparability no longer clears points). `backfill_benchmark_prices.py` (index closes, not user holdings) is unaffected. **Issue #398**: `backfill_fx_rates.py` seeds ~5y of daily `fx_rates` for every `_PAIRS` entry (idempotent upsert, no `is_backfilled`) so non-USD Performance can draw full selected-range benchmarks; daily `capture_fx_task` stays the freshness path. Do not revive composition-replay (#366/#367). **Issues #402/#403** (production verification of #398/#400, 2026-09-09): both backfill scripts' bulk upserts hit PostgreSQL's 65,535-parameter limit at real 5-year data volume — see the Known-fixed bugs entry above, fixed via PR #404/#405. **Issue #406** (open): `USDCNH` specifically has only ~1 month of yfinance history regardless of fetch strategy (confirmed a genuine vendor limitation, not a code defect — Sina's historical forex API is broken for any pair, not just CNH; no working historical fallback found yet). **Issue #407** (open): `csi300`'s yfinance history (`000300.SS`) had a real ~2-month gap (2026-07-18 to 09-08) — manually back-filled in production from a verified Tencent kline fallback (cross-checked against Yahoo anchors, agreement to 4 significant figures), but the underlying yfinance unreliability is not yet wired into the automated capture/backfill path, so the gap can recur.
- [Per-pair independent FX rate resolution + fx_rates_as_of](docs/mechanisms/capture-and-reporting.md) — issue #354: `_load_fx_rates()` resolves each FX pair's own latest `rate_date` independently instead of requiring one shared date across all 14 pairs (the actual root cause of holdings silently dropping out of `/portfolio` depending on the selected base currency — `_to_base()`'s two-layer aggregation was already correct). `PortfolioSnapshot.fx_date` retired in favor of `fx_rates_as_of: dict[str, date]` (per non-USD currency actually needed this render), threaded through `GET /portfolio/summary`, report text (`format_fx_rates_as_of()`), and a new page-level `FxAsOfBanner` (sibling of `PriceAsOfBanner`) that discloses mixed-date pairs rather than forcing alignment. `fx_fetcher.py` gained two distinct ops alerts (per-pair fetch failure; request-time resolvable-rate gap/staleness — the latter is what actually catches this issue's failure mode), both reusing `alert_dedup.py`'s #298 pattern. `currency-switcher.tsx` rebuilt on `MenuDropdown`/`MenuItemButton` (`LocaleSwitcher` precedent) with flag-icons; `currencies.ts` split into `BASE_CURRENCIES` (15, unchanged, still backs `/profile`'s unrelated report-currency setting), `PORTFOLIO_DISPLAY_CURRENCIES` (7, the switcher's full menu), and `PORTFOLIO_NORMALIZATION_TARGETS` (3: USD/CNY/HKD, the subset actually clickable — the other 4 show with a greyed-out flag, disabled not hidden). Both ops alerts are gated on `Settings.APP_ENV == "production"` (2026-09-05 follow-up, same field `db_backup.py` uses) — a local dev Postgres has no Celery beat keeping `fx_rates` fresh, so it reads as permanently stale by design and would otherwise spam the real admin inbox on manual testing.
...
- [Price-fetch resilience: typed errors, telemetry, Finnhub + Massive.com fallbacks](docs/mechanisms/capture-and-reporting.md) — issue #56: `app/services/price_errors.py`'s `PriceFetchErrorCode` (`rate_limit`/`connection`/`no_data`), modeled on `llm_errors.py`'s `LLMErrorCode`/`ErrorPolicy` split (issue #55) — classification only, `price_fetcher.py`'s existing retry loops untouched; structured `logger.info` provider telemetry at every fetch call site (log-shape only, no table); `_yfinance._quiet_yfinance_logs()` demotes the `yfinance` logger during download calls to kill a known false "possibly delisted" ERROR. `app/services/_finnhub.py` (`fetch_quotes`) — US-only spot fallback (verified live: free tier rejects non-US symbols), wired into `price_capture.py`'s non-close branch only, gated on `market == "US"` + `Settings.FINNHUB_API_KEY`, shares a free-tier rate-limit bucket with the unrelated Daily_Intelligence project's key (accepted). `app/services/_massive.py` (`fetch_prev_close_ohlcv`) — Massive.com (formerly Polygon.io) free-tier EOD-only US fallback (same-day data withheld, T-1 onward available), wired into the close branch only, gated on `market == "US"` + `Settings.MASSIVE_API_KEY`; both fallbacks route every price through the shared `_safe_scaled_price` gate (issue #204/#311 GBX precedent) and are fail-open per ticker. No fetcher-interface abstraction, no per-source health tracking — both real fallbacks are simple market/node conditionals, not a health-tracked source pool (deliberately deferred, see the mechanism doc's exclusion list).
...
- [Report footer disclaimer: single-language, not always bilingual](docs/mechanisms/report-content-and-email.md) — issue #350 item 3: `report_sections._build_footer` gains an `output_lang` parameter (default `en`) and renders only that locale's copy instead of always emitting both English and zh-Hans — both call sites (`report_generator.py`'s full-report path, `email_sender.py`'s portfolio-overview email) thread the real value through.
...
- [Reliability mechanisms (window/dedup/LLM-call correctness)](docs/mechanisms/llm-reliability.md) — same-day windows, Pass 2 completeness guard, `_call_llm` retry/backoff, `session_node`. Issue #61: retry of a `failed`/`in_progress` row with a reusable stored `pass2_raw`/`assembly_raw` (matching `prompt_version`/`disclaimer_version`, not `needs_review`) skips Pass 1 + Pass 2 and resumes from render via the extracted `_finish_report()` tail; a `status=="success"` row with `email_sent_at IS NULL` gets a resend-only path with no re-render/LLM call.
...
- **Bulk `INSERT ... VALUES` upserts must chunk, not go in one statement**
  (issues #402/PR #404, #403/PR #405): PostgreSQL/psycopg cap a single
  statement at 65,535 bound parameters. `fx_fetcher._upsert_fx_history` and
  `benchmark_prices._upsert` both built one unbounded multi-row `INSERT`
  from every fetched row — fine for a handful of rows, but a real multi-year
  backfill (14 FX pairs / 4 benchmark indexes × ~5 years daily) blew past the
  limit and crashed before any row committed. `price_capture.py` already had
  this fix from issue #194; these two call sites hadn't picked it up. Fixed
  by chunking into fixed-size batches (5,000 rows for FX, 2,000 for
  benchmarks) — if you add a new bulk-upsert call site with more than a
  couple thousand rows, chunk it from the start rather than waiting to hit
