...
- [Idle-timeout server enforcement](docs/mechanisms/identity-and-auth.md) — issue #235: the 15-min auto-logout (issue #207) was client-only and silently defeated by closing the browser; `current_principal` now checks a Redis-stored last-active timestamp (`app/core/idle_activity.py`), fail-open on Redis outage (deliberate departure from rate_limit's fail-closed convention — blast radius, see mechanism doc); absolute session lifetime (Supabase refresh-token expiry) and per-user configurable length are explicitly out of scope here. PR #240 review round 1 (blacktomb42) found 2 ship-blockers, both fixed: a real re-login after an idle 401 stayed 401 for 24h (fixed via the token's own `iat` claim overriding a stale record); the 401 never actually signed the browser out (fixed by routing `lib/api.ts`/`lib/server-api.ts` 401s through the same `logout()` Server Action the client idle timer uses, across 8 call sites).
