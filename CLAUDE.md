...
- [Ops user hard-purge](docs/mechanisms/identity-and-auth.md) — issue #199/#225: `DELETE /admin/users/{id}?confirm={email}` hard-deletes one user's own rows and now also the Supabase Auth account (sequenced before local deletes, 502+no-op on failure); also handles Auth-only orphans with no local row.
