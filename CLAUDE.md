...
| Identity (B4, issue #129, PR #183) | Request identity is `Depends(current_principal)` only: `Authorization: Bearer` verified against hosted Auth JWKS (ES256/RS256; **no `JWT_SECRET` Settings field**), then `users.auth_subject` + `status=active`. Missing/forged/unknown-`sub` → 401, **no auto-insert**. `get_current_user_id()` raises. Ops `/admin/*` is a separate bearer (`ADMIN_API_TOKEN`), not this JWT. **Do not deploy this cutover without B5** — unauthenticated `/holdings` `/reports` `/portfolio` now 401. `current_principal` also enforces a 15-minute rolling idle timeout (issue #235) and an 8-hour absolute session lifetime cap (issue #236, permanent app-level — Supabase's own Time-box is Pro-tier-only and inert on this project's Free plan); `GET /auth/session-status` is a cheap 204 probe the frontend top-bar menu calls to avoid showing authenticated entries once either has expired. |
...
- [`watch_tier`: watched holdings with a config-driven §3 floor](docs/mechanisms/holdings-pipeline.md) — issue #421/#424, PR #425/#427: `watch_tier` CHECK column + `watch_tier_weights.yml`, `max(real_weight, config_weight)` floor into #173's check (never an absolute substitute), excluded from export/template/`confirm`/portfolio-export dialects — `confirm_holdings` strips it per-row, not inside the shared insert helper `create_holding` also uses; issue #430 added the watch-tier icon UI (`/portfolio`, `/holdings`, inline edit on `/holdings/edit`) and a new `/holdings/groups` bulk group/account editor.
...
- [Portfolio Performance — Phase 1 + tracking-start + holiday history + param-limit fixes](docs/mechanisms/portfolio-performance.md) — issue #360/#382/#383/#366/#377/#398/#402/#403/#406/#407/#433/#486/#487: snapshot tables, approximate EOD TWR, `tracking_start` correction (#366), benchmark/FX multi-year seed scripts, both scripts' 65535-param chunking fix, #406 USDCNH history (Twelve Data, closed), #407 csi300 Tencent kline fallback when yfinance drops dates, #433 asset-class allocation history + monthly performance (single-benchmark) + CSI300 Performance-page color fix, #486 null calendar-row insertion across missing snapshot dates (the per-gap dashed columns were removed in #544: one solid portfolio line with connectNulls; `formatTickPct` is fixed at two decimals), #487 every-day capture + `approx_carried` disclosure + fingerprint-gated weekend backfill script.
- [Portfolio historical volatility and risk](docs/mechanisms/portfolio-risk.md) — issues #551/#553/#555: read-only NYSE-sampled portfolio volatility and fixed S&P Beta, multi-benchmark volatility, questionnaire-relative Risk, and holdings-structure Deviation on Portfolio Overview. Issue #557: the logged-out homepage risk preview composes components exported from `risk-panel.tsx` with static sample data and re-declares the chart JSX, so changes to those exports or to the panel's chart settings must be mirrored there.
...
- [Capture-health human-readable alert + FX live-quote capture](docs/mechanisms/portfolio-performance.md) — issue #426: `CaptureHealthReport` carries each pipeline's last-success date, alert body rewritten to plain-English blocks. Issue #519 (supersedes #426's retry+fallback framing and #509's same-day retime): both FX providers switched from a daily-close/daily-bar fetch to a live-quote fetch (`fetch_live_rate` on yfinance's `fast_info` and Twelve Data's `/price`) — a 24/5 FX market has no real daily close to wait for, so `capture_fx_task` (16:00 ET) and `capture_fx_evening_task` (20:00 ET, was `capture_fx_catchup_task`) are now two symmetric attempts, neither a catch-up of the other, each falling back to Twelve Data (rate-limited) in the same run if yfinance misses a pair. `_load_fx_rates` (`portfolio_calculator.py`) gained an independent 48h read-time freshness flag (`stale_fx_pairs`) alongside the unchanged 10-day `historical_fx_rates_asof` lookback and 2-business-day `approx_carried` tolerance. `backfill_weekend_gaps.py` retired (its one historical gap was already backfilled).
- [Per-pair independent FX rate resolution + fx_rates_as_of](docs/mechanisms/capture-and-reporting.md) — issue #354: per-pair `fx_rates_as_of`, `FxAsOfBanner`, currency-switcher rebuild on `MenuDropdown`. Issue #532/PR #547: that same banner warns when a displayed currency is in backend `stale_fx_pairs` (fetched more than 48 hours ago); the client does not recompute the threshold, and performance charts stay unchanged.
...
- [Report content features](docs/mechanisms/report-content-and-email.md) — Ring0 #1-4 + R-3/R-5/R-6/R-7/R-8: §4.2 anomaly table, confidence labels, §4.4 technical position, §2.5 forward calendar. Issue #492 (PR #500): `window_data._merge_theme_anomalies._val()` weighted holdings sharing a theme by raw `Holding.current_value`, which is never a real valuation for `pricing_mode="auto"` holdings — now `shares × price` for auto, unchanged for manual/cash. Forward-only, no retroactive report regeneration.
...
- [§3 proportionality check: log-only length-vs-(weight, evidence)](docs/mechanisms/report-content-and-email.md) — issue #173: `section3_proportionality.py`, keyword-scored evidence strength, paragraph+identifier §3 segmentation (no per-holding heading exists), weight as an explicit parameter, wired into `_render_full_md`.
...
- [Compliance + ops alerting](docs/mechanisms/compliance-and-classification.md) — forbidden-vocab scan, disclaimer, `send_ops_alert`, GitHub issue auto-creation; issue #443/PR #444 EN scan directive/own-voice shared builders, five review rounds — **known structural limit, not a closed bug list: read the mechanism doc's "Known limitation" note before opening a new issue for the next false positive/negative on this scan.**
...
- **`data_collection=deny` on every LLM call by default**, as defense in
  ...
  `data_collection=deny` — not an `allow_fallbacks=True` on the BYOK leg.
  *Full reasoning: playbook `docs/playbooks/llm-and-data-handling.md`.*
