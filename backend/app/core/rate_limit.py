...
def _forwarded_client_ip(request: Request) -> str | None:
    """Leftmost X-Forwarded-For hop, else X-Real-IP. None if neither is set."""
...
def client_id_from_request(request: Request) -> str:
    """Prefer Caddy's X-Forwarded-For / X-Real-IP over the ASGI peer.

    UI signup is Next.js → backend; the peer is the frontend container.
    TestClient does not apply uvicorn --proxy-headers, so the limiter reads
    these headers itself (issue #190 / PR #222).
    """
    forwarded = _forwarded_client_ip(request)
    if forwarded is not None:
        return canonical_client_id(forwarded)
