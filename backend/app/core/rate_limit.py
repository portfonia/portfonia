...
def _note_global_signup() -> None:
    ...
    key = f"rl:signup:global:{day}"
    try:
        n = get_backend().incr_with_ttl(key, SIGNUP_GLOBAL_TTL)
    except RateLimitUnavailable:
        logger.exception("rate_limit: global signup counter unavailable")
        return
    except Exception:
        logger.exception("rate_limit: global signup counter failed")
        return
    if n >= SIGNUP_GLOBAL_ALERT_LIMIT:
        _maybe_alert(
            scope="signup-global",
            bucket=day,
            window=SIGNUP_GLOBAL_TTL,
            subject="Portfonia ops: signup volume circuit",
            body=f"{n} signup attempts today (UTC {day}); threshold {SIGNUP_GLOBAL_ALERT_LIMIT}. Not auto-blocked.",
        )
