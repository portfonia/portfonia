...
def test_compute_global_moves_normalizes_known_collision_ticker(db_session: Session) -> None:
    """issue #204 PR #253 review: a holding declared ticker='700.HK' (raw,
    un-padded) must produce a move keyed '0700.HK' — the same normalized
    key price_capture writes closes under and select_user_anomalies looks
    moves up by. Before this fix, this function's identifier universe (via
    global_identifier_universe) stayed keyed under the raw form, so it
