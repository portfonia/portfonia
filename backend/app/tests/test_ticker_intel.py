...
def test_holding_identifier_normalizes_known_collision_ticker() -> None:
    ...
    assert ti._holding_identifier({"ticker": "700.HK"}) == "0700.HK"


def test_build_l1_facts_normalizes_known_collision_ticker_before_matching_technical_positions() -> (
    None
):
    """Same shape as the HK-normalization case above:
    technical_positions[].ticker is the raw Holding.ticker ('700.HK'), while
    identifiers/day_moves keys are the normalized form ('0700.HK')."""
    identifiers = ti.l1_identifiers_for_user([{"identifier": "0700.HK", "constituents": []}])
    technical = [{"ticker": "700.HK", "pct_vs_sma50": 0.04}]
    facts = ti.build_l1_facts(identifiers, {"0700.HK": _move("0700.HK")}, {}, technical)
    assert facts["0700.HK"].pct_vs_sma50 == 0.04
