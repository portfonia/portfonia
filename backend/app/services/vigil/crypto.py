"""Dedicated Vigil data-key contextual Fernet (issue #454, Vigil R0 P2.1).
...
requires a fully independent key family, and Vigil's Fernet payloads must
...
wrapping) as holdings encryption, but a separate key and a separate module so
the two systems' failure domains never overlap and neither implementation
change can silently affect the other.

Only the "data" key family (VIGIL_ENCRYPTION_KEY/_PREV) is implemented here —
the "notification" family (VIGIL_NOTIFICATION_KEY/_PREV) protects the
short-lived mail outbox payload and isn't needed until #456.
"""
...
class VigilCryptoError(RuntimeError):
    """Raised for a missing/malformed Vigil data key, an undecryptable
    token, an unrecognized/malformed envelope, or a context mismatch
    (wrong purpose/table/row_id/vault_id) between the stored ciphertext and
    the caller's expected identity. Never includes the token or decrypted
...
def _build_fernet() -> MultiFernet:
    ...
    key = settings.VIGIL_ENCRYPTION_KEY
    if key is None or not key.get_secret_value():
        raise VigilCryptoError(
            "VIGIL_ENCRYPTION_KEY is not configured — Vigil crypto is unavailable."
        )
    try:
        keys = [Fernet(key.get_secret_value().encode())]
        prev = settings.VIGIL_ENCRYPTION_KEY_PREV
        ...
        raise VigilCryptoError(
            "VIGIL_ENCRYPTION_KEY / VIGIL_ENCRYPTION_KEY_PREV is not a well-formed "
            "Fernet key — check the deployed .env value."
        ) from exc
    return MultiFernet(keys)


def _encrypt_envelope(envelope: dict[str, Any]) -> str:
    payload = json.dumps(envelope, separators=(",", ":"), sort_keys=True)
    return _build_fernet().encrypt(payload.encode()).decode()


def _decrypt_envelope(token: str) -> dict[str, Any]:
    try:
        raw = _build_fernet().decrypt(token.encode())
...
def encrypt_field(value: str, *, purpose: str, table: str, row_id: UUID, vault_id: UUID) -> str:
    """Encrypt `value` under a context envelope binding it to exactly this
    (purpose, table, row_id, vault_id). `value` must already be a string —
    callers JSON-encode/base64url-encode richer structures themselves
    before calling this."""
    envelope = {
    ...
    return _encrypt_envelope(envelope)


def decrypt_field(token: str, *, purpose: str, table: str, row_id: UUID, vault_id: UUID) -> str:
    """Decrypt `token`, then reject it unless its envelope's context exactly
    matches the caller's expected (purpose, table, row_id, vault_id) — this
    is what stops a ciphertext copied onto a different row from silently
    decrypting (A02/A04)."""
    envelope = _decrypt_envelope(token)
